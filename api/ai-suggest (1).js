...
//    Name:  NVIDIA_API_KEY
//    Value: nvapi-xxxxxxxxxx   (your NVIDIA NIM API key from build.nvidia.com)
// ============================================================
...
export default async function handler(req, res) {
  ...
  if (!process.env.NVIDIA_API_KEY) {
    ...
    return res.status(500).json({ error: 'AI service is not configured. Add NVIDIA_API_KEY in Vercel environment variables.' });
  }
...
Respond ONLY with valid JSON:
{
  ...
  try {
    ...
    if (!nvidiaRes.ok) {
      ...
      console.error('NVIDIA API Error status:', nvidiaRes.status, errText);
      // Surface a meaningful message for common errors
      if (nvidiaRes.status === 401) {
        return res.status(502).json({ error: 'Invalid NVIDIA API key. Check your NVIDIA_API_KEY in Vercel.' });
      }
      if (nvidiaRes.status === 429) {
        return res.status(502).json({ error: 'NVIDIA API rate limit reached. Please wait a moment and try again.' });
      }
      return res.status(502).json({ error: `AI service error (${nvidiaRes.status}). Try again.` });
    }
    ...
    if (!rawText) {
      console.error('Empty response from NVIDIA:', JSON.stringify(nvidiaData));
